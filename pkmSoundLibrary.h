...
class pkmSoundLibrary
{
public:
	pkmSoundLibrary(int sr = 44100, int ws = 512)
	{
		sample_rate = sr;
		window_size = ws;
	...
	~pkmSoundLibrary()
	{
		for(vector<pkmSoundObject *>::iterator it = soundLibrary.begin(); it != soundLibrary.end(); it++)
		{
			delete *it;
		}	
#if(DO_INTERLEAVED)
	...
	void addSound(float *sound_data, int num_samples, 
				  ...
				  double lon = 0.0)
	{
		pkmSoundObject *soundObject = new pkmSoundObject(sample_rate, window_size);
	...
	void audioRequested()
	{
		...
		//if (bEncodedLiveObject) 
		//{
		//	liveSignalObject->updateCurrentBinauralWindow();
		//	if (liveSignalObject->isAudible()) 
		//	{
		//		leftChannels.push_back(liveSignalObject->getBinauralL());
		//		rightChannels.push_back(liveSignalObject->getBinauralR());
		//	}
		//}
		...
		if (numAudibleSounds > 0) 
		{
			...
			for (int i = 0; i < window_size; i++) 
			{
				...
				{
					bufferPtr[0] += leftChannels[s][i];
					bufferPtr[1] += rightChannels[s][i];
				}
				bufferPtr[0] /= (float)numAudibleSounds;
				bufferPtr[1] /= (float)numAudibleSounds;
			...
			for (int i = 0; i < window_size; i++) 
			{
				for(int s = 0; s < numAudibleSounds; s++)
				{
					binauralL[i] += leftChannels[s][i];
					binauralR[i] += rightChannels[s][i];
				}
				binauralL[i] /= (float)numAudibleSounds;
				binauralR[i] /= (float)numAudibleSounds;
			}
	#endif
		}
		else 
...
private:
	...
	vector<pkmSoundObject *>		soundLibrary;
	int								sample_rate;
	int								window_size;
